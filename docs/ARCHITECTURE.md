...
While recording, the popup consumes a derived, read-only live-status contract
from the recorder background service. It reports capture progress, connection,
Business Central context and bounded warnings without reading or changing raw
...
### Final page-identification integration review

The release review verified the complete path from observable recorder facts to
Canonical Recording, in-memory identification, normalization, Step Groups,
Semantic Actions, Review, Semantic Document, Document Plan, Workspace, and Word.
Renderers consume the validated plan and neither classify pages nor reconstruct
page semantics. Screenshot Intelligence uses stable page identity for continuity
only. `src` remains authoritative; `dist` is regenerated by `scripts/build.js`.

Representative standard, localized, unknown, legacy, Aptean-fixture, customer,
and frame/control-add-in flows completed without losing raw evidence or exposing
sensitive query/business values in identification diagnostics. The verified
configured registry remains deliberately small; semantic-only warehouse,
manufacturing, and Aptean rules are not misrepresented as verified page records.
