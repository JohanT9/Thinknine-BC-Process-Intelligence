...
assert.strictEqual(checked.dataset.diagrams.length, 43);
...
assert.deepStrictEqual(seed.diagrams.find(item => item.name === "Sales Return").processGraph.nodes
  .filter(item => !["start", "end"].includes(item.nodeType)).map(item => item.nodeType),
  ["document", "posting", "postedDocument", "document", "postedDocument"]);
assert.deepStrictEqual(seed.diagrams.find(item => item.name === "Inventory Pick").processGraph.nodes
  .filter(item => !["start", "end"].includes(item.nodeType)).map(item => item.nodeType),
  ["document", "posting", "postedDocument"]);
assert.deepStrictEqual(seed.diagrams.find(item => item.name === "Transfer Order").processGraph.nodes
  .filter(item => !["start", "end"].includes(item.nodeType)).map(item => item.nodeType),
  ["document", "posting", "postedDocument", "posting", "postedDocument"]);
assert(seed.diagrams.find(item => item.name === "Item Transfer").processGraph.nodes.some(node =>
  node.taxonomyEntityIds?.includes("concept:create-transfer-order")));
assert.strictEqual(seed.diagrams.find(item => item.name === "Physical Inventory")
  .processGraph.nodes.find(item => item.title === "Record Physical Count").nodeType,
"manualAction");
...
["document", "processStep", "document", "processStep", "document", "processStep",
  "postedDocument"]);
assert.deepStrictEqual(["Production Consumption", "Production Output",
  "Production with Warehouse Picking", "Production with Warehouse Put-away"].map(name =>
  seed.diagrams.some(item => item.name === name)), [true, true, true, true]);
assert(seed.diagrams.find(item => item.name === "Production with Warehouse Picking")
  .processGraph.nodes.some(node => node.taxonomyEntityIds?.includes("document:warehouse-pick")));
assert(seed.diagrams.find(item => item.name === "Production with Warehouse Put-away")
  .processGraph.nodes.some(node => node.taxonomyEntityIds?.includes("document:warehouse-put-away")));
assert(seed.diagrams.find(item => item.name === "Make to Stock").processGraph.nodes.some(node =>
  node.taxonomyEntityIds?.includes("document:planning-worksheet")));
assert(seed.diagrams.find(item => item.name === "Make to Order").processGraph.nodes.some(node =>
  node.taxonomyEntityIds?.includes("document:sales-order")));
...
assert.deepStrictEqual(["Create Purchase Orders", "Create Production Orders",
  "Create Transfer Orders"].map(name => seed.diagrams.find(item => item.name === name)
  ?.processGraph.nodes.at(-2).nodeType), ["document", "document", "document"]);
...
assert(seed.diagrams.find(item => item.name === "Order Planning").processGraph.nodes.some(item =>
  item.taxonomyEntityIds?.includes("document:order-planning")));
assert(seed.diagrams.find(item => item.name === "Purchase Receipt and Invoice")
  .processGraph.nodes.some(item => item.nodeType === "postedDocument" &&
    item.taxonomyEntityIds?.includes("document:posted-purchase-invoice")));
assert(seed.diagrams.find(item => item.name === "Assembly to Order").processGraph.nodes.some(item =>
...
assert.strictEqual(published.diagrams.length, 44);
