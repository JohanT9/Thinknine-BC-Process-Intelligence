  ...
  return new Paragraph({
    ...
    children: [
      new TextRun({
        text: `${component.content.label}: `,
        bold: true,
        color: color(component.appearance.labelColor,
...
function renderComponent(component, mediaAssets, context = {}) {
  ...
  if (component.kind === "step") {
    const instructionIndex = component.components.findIndex(child =>
      child.kind === "paragraph");
    const instruction = instructionIndex >= 0
      ? component.components[instructionIndex]
      : null;
    const supportingComponents = component.components.filter((child, index) =>
      index !== instructionIndex);
    return [
      stepLeadBlock(component, instruction),
      ...supportingComponents.flatMap(child =>
