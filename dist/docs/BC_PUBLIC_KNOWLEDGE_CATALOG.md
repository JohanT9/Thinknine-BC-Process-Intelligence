...
There are 203 distinct page IDs across both snapshots. The enabled process packs now contribute 88 source-linked observable-action rules, including sales and purchase return/correction flows, customer/vendor payments, and payment reconciliation. Selected sales and purchase order-line, shipping/receiving, release and posting rules, return/correction, cash/payment, warehouse pick/receipt/put-away, and production-journal posting/output-quantity rules now have runtime-tested matching coverage across all eight supported UI locales. Production-consumption quantity fields, other warehouse rules, and the broader inventory and core-action rules still have narrower language coverage. Some page IDs appear in both versions, because captions and metadata can change. For example, page 22 is documented as `Customer List` in the BC 21.0 snapshot and `Customers` in the BC 28.x reference. The catalog intentionally returns no version-scoped fact for gaps such as BC 26 when no matching source has been included.
